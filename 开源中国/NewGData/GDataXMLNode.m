...
{
    NSArray *array=[self elementsForName:subName];
...
-(NSArray*)stringValuesByName:(NSString *)subName
{
    ...
    for (GDataXMLElement *element in array) {
        [valuesArray addObject:[element stringValue]];
    }
    return valuesArray;
}
-(NSString*)attributeValueByName:(NSString *)attrName
{
    GDataXMLNode *node=[self attributeForName:attrName];
    return [node stringValue];
}
//获得当前节点指定名称所有子节点（数组中为要获得的所有子节点的名称）的值
-(NSDictionary *)subDictWithArray:(NSArray*)subNames{
    NSMutableDictionary *dict = [NSMutableDictionary dictionary];
    //把数组中 当前节点的 子节点名和对应的子节点的值 存入字典
    for (NSString *subName in subNames) {
        NSArray *subs = [self elementsForName:subName];
        //增加键值对
        [dict setObject:[subs[0] stringValue] forKey:subName];
    }
    return dict;
}
...
{
    GDataXMLDocument *doc=[[GDataXMLDocument alloc] initWithXMLString:@"<?xml version=\"1.0\" encoding=\"utf-8\"?><root></root>" encoding:NSUTF8StringEncoding error:nil];
    return doc;
}
//根据dict字典创建xml节点
//@"name" : @"呐喊"

-(void)xmlElementFromDict:(NSDictionary*)dict element:(GDataXMLElement *)element
{
    for (NSString *key in [dict allKeys] ) {
        id object=[dict objectForKey:key];
        if ([object isKindOfClass:[NSDictionary class]]) {
            //创建子节点
            GDataXMLElement *chiledElement=[GDataXMLElement elementWithName:key];
            //递归 创建下级节点
            [self xmlElementFromDict:object element:chiledElement];
            [element addChild:chiledElement];
        }
        else if([object isKindOfClass:[NSString class]]){
            //根据字典的键值创建xml节点
            GDataXMLElement *chiledElement=[GDataXMLElement elementWithName:key stringValue:object];
            [element addChild:chiledElement];
        }
    }
}
